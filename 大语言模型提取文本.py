...
# Ensure you have downloaded the punkt tokenizer data
nltk.download('punkt')

# Step 1: Extract sentences from PDF using PyMuPDF
def extract_sentences_from_pdf(pdf_path):
    try:
        ...
        for page_num in range(len(document)):
            ...
            # 过滤掉表格和图片中的文本
            text_blocks = [block[4] for block in blocks if block[6] == 0]  # 0表示普通文本
...
# Step 2: Use BERT to find sentences containing keywords
...
def save_results_to_excel(keyword_sentences, output_file):
    df = pd.DataFrame(keyword_sentences, columns=['Sentences'])
    df.to_excel(output_file, index=False)

# Main function to run the process on all PDF files in a folder
def main(folder_path, keywords, output_folder):
    # 创建输出文件夹（如果不存在）
    ...
    for filename in os.listdir(folder_path):
        if filename.endswith('.pdf'):
            ...
            output_file = os.path.join(output_folder, f"{os.path.splitext(filename)[0]}_output.xlsx")

            # Extract sentences from PDF
            sentences = extract_sentences_from_pdf(pdf_path)

            # Find sentences containing keywords
            keyword_sentences = find_sentences_with_keywords(sentences, keywords)

            # Save results to an Excel file
            save_results_to_excel(keyword_sentences, output_file)

            # Print the number of sentences found
            print(f"Processed {filename}: Found {len(keyword_sentences)} sentences containing the keywords.")

# Example usage
if __name__ == "__main__":
    folder_path = r'C:\Users\HP\Desktop\易旻小组\供应商\黑色金属矿采选业\黑色金属矿采选业'  # Replace with your folder path containing PDF files
    keywords = [
        ...
        '碳酸钠', '氢氧化钠', '锶濟及其化合物', '锶', '饈鉤嬤觜樣友化锶', '氧化锶', '钽及其化合物', '檗',
        ...
        '废盐', '工程渣土', '工程泥浆', '工程垃圾']  # Replace with your keywords
    output_folder = r'C:\Users\HP\Desktop\易旻小组\供应商\put'  # Output folder path

    main(folder_path, keywords, output_folder)